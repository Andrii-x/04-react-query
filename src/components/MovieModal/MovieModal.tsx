...
function MovieModal({ movie, onClose }: MovieModalProps) {
  ...
  return <div className={css.backdrop} role="presentation" onClick={handleBackdropClick} onKeyDown={handleBackdropKeyDown}><div className={css.modal} role="dialog" aria-modal="true" aria-labelledby="movie-title"><button className={css.close} type="button" onClick={onClose} aria-label="Закрити">×</button>{movie.poster_path && <img className={css.poster} src={`${posterBaseUrl}${movie.poster_path}`} alt={`Постер фільму ${movie.title}`} />}<div className={css.content}><p className={css.year}>{releaseDate} <span>·</span> Оцінка {movie.vote_average.toFixed(1)}</p><h2 id="movie-title">{movie.title}</h2><p className={css.overview}>{movie.overview || 'Опис цього фільму поки що відсутній.'}</p></div></div></div>
}
